...
@section('body')
    <div class="container-fluid p-5" style="width:100%;">
        ...
        <div class="mb-5 text-center schedule">
            <form action="{{ route('class-modify', ['id' => $schedule->id]) }}" method="POST">
                @csrf
                <div class="time-container">
                    <label for="timestart">Waktu Mulai</label>
                    <input id="timestart" type="time" name="timestart" value="{{ $schedule->start }}" />
                </div>
                <div class="time-container">
                    <label for="timeend">Waktu Berakhir</label>
                    <input id="timeend" type="time" name="timeend" value="{{ $schedule->end }}" />
                </div>
                <div class="time-container" style="margin: 10px 0;">
                    ...
                    <select name="timeday" id="timeday" style="width: 120px; padding: 10px; border-radius: 15px;">
                        @foreach ($hari as $h)
                            <option @if ($schedule->hari == $h) selected @endif value="{{ $h }}">
                                {{ $h }}</option>
                        @endforeach
                    </select>
                </div>
                <div class=""
                    ...
                    <textarea style="width: 300px;" required id="timedesc" name="timedesc">{{ $schedule->description }}</textarea>
                </div>
